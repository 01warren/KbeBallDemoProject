...
public:
	...
	*/
	/**
		@param fromExternal: Router 模式下没有 Channel，由派发方显式声明调用来自客户端(外部)。
		@param externalSrcEntityID: 当 fromExternal 为 true 且无 Channel 时，调用方的实体 id
			(取自 Mail.src)。老路径下由 pChannel->proxyID() 得到，二者语义一致。
	*/
	void onRemoteMethodCall(Network::Channel* pChannel, MemoryStream& s,
		bool fromExternal = false, ENTITY_ID externalSrcEntityID = 0);
