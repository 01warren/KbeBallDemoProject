...
protected:
	...
	// Router 模式下客户端不与 baseapp 直连，待发送的 Bundle 只能缓存在 proxy 本地
	// (老路径缓存在 Network::Channel::bundles_ 上)。仍然在 sendToClient(bool) 时统一 flush，
	// 以保留原有的发包节流(proxy_forwarder)语义。
	Bundles routerBundles_;
