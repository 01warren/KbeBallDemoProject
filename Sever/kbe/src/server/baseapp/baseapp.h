...
public:
	...
	*/
	...
	//
	// 客户端只连 Router，baseapp 侧用 MAILBOX_ENTITY{proxyId, componentType=CLIENT_TYPE}
	// 表示"该实体对应的客户端连接"(见 docs/kbe_router_design.md §9.2)。
	// ------------------------------------------------------------------

	/** 把某实体的客户端 Actor 绑定到它在 router 上的连接(登录/重登录时调用)。
		@param clientRouterConnId 客户端在 router 上的 connId(由客户端 HELLO 得到)；
		                          为 0 表示尚不可知，此时不做绑定(保持旧通路)。 */
	bool bindClientProxyActor(ENTITY_ID eid, uint64_t clientRouterConnId);

	/** 注销某实体的客户端 Actor(客户端断线 / 被踢 / 转移给其他 proxy)。
		注意：重新绑定前必须先注销，否则 router 路由表会残留旧映射。 */
	void unbindClientProxyActor(ENTITY_ID eid);

	/** 经 Router 向某实体的客户端投递一段"拍平的 legacy interface 消息载荷"。
		由 Proxy::sendToClient 在 Router 模式下调用，body tag 为 RouterMail::BODY_CLIENT_MESSAGE。 */
...
protected:
	...
	//
	// base 实体 Actor 地址只由 entityID 决定；base 部分(宿主 baseapp)与 cell 部分
	// (宿主 cellapp)是同一 entityID 下的两个 Actor，Router 侧靠 componentType 区分。
	// ------------------------------------------------------------------

	/** entity 创建后注册 base 实体 Actor */
	virtual void onEntityCreated(Entity* entity);

	/** entity 销毁前注销 base 实体 Actor */
	virtual void onEntityDestroyed(Entity* entity);

	/** router(重)连成功：重建本进程全部实体 Actor */
