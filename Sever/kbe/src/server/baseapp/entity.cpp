...
void Entity::sendToCellapp(Network::Bundle* pBundle)
{
	...
	// Router 模式：不再有"到 cellapp 的直连频道"，把这条组件级裸消息投递到
	// 目标 cellapp 的组件 Actor(载荷与旧通路逐字节相同，对端 onRouterMail 按
	// msgId 派发回原 CellappInterface handler)。
	if(RouterMail::isEnabled())
	{
		Baseapp::getSingleton().sendMailToComponent(cellEntityCall_->componentID(),
...
void Entity::onRemoteMethodCall(Network::Channel* pChannel, MemoryStream& s,
	bool fromExternal, ENTITY_ID externalSrcEntityID)
{
	SCOPED_PROFILE(SCRIPTCALL_PROFILE);
	...
	// 如果是外部通道调用则判断来源性
	// Router 模式下无 Channel，由派发方以 fromExternal=true + externalSrcEntityID 表达同一语义，
	// 因此"只允许客户端调用自己的实体"这一校验在两种路径下都被保留。
	if (fromExternal || (pChannel != NULL && pChannel->isExternal()))
	{
		ENTITY_ID srcEntityID = (pChannel != NULL) ? pChannel->proxyID() : externalSrcEntityID;
...
void Entity::onMigrationCellappStart(Network::Channel* pChannel, COMPONENT_ID sourceCellAppID, COMPONENT_ID targetCellAppID)
{
	...
	if (hasFlags(ENTITY_FLAGS_TELEPORT_STOP))
	{
		...
		// Router 模式下不会创建客户端转发缓冲(见 onMigrationCellappEnd 的说明)，
		// 只有旧通路下才要求它必须存在。
		if(RouterMail::isEnabled())
		{
...
void Entity::onMigrationCellappEnd(Network::Channel* pChannel, COMPONENT_ID sourceCellAppID, COMPONENT_ID targetCellAppID)
{
	...
	{
		// Router 模式：cellapp -> client 已改为经 Router 直投 client Actor(不再由
		// baseapp 中继)，因此不存在"迁移窗口内 baseapp 必须先缓存客户端消息再补发"
		// 的乱序场景，也就不需要 BaseMessagesForwardClientHandler。顺序由 Router
		// 的单连接 FIFO 与 cell 实体 Actor 的 SUSPEND/RESUME 保证。
		if(RouterMail::isEnabled())
		{
...
void Entity::onMigrationCellappOver(COMPONENT_ID targetCellAppID)
{
	...
	// Router 模式：组件不再两两直连，改为投递到目标 cellapp 的组件 Actor
	if(RouterMail::isEnabled())
	{
		Baseapp::getSingleton().sendMailToComponent(targetCellAppID, CELLAPP_TYPE, pBundle);
	}
	else
	{
