...
void Entity::sendControlledByStatusMessage(EntityCall* baseEntityCall, int8 isControlled)
{
	...
	// Router 模式下客户端不再与 cellapp 直连(pChannel 恒为 NULL)，改为经 Router 直投 client Actor
	...
	// 直投 client Actor 时目标必须是"控制者的客户端"(与中继包里的 entityID 一致)，
	// 而不是被控制实体自己的客户端。
	if(!Cellapp::getSingleton().sendBundleToClientActor(baseEntityCall->id(), pSendBundle))
...
void Entity::onDefDataChanged(EntityComponent* pEntityComponent, const PropertyDescription* propertyDescription, PyObject* pyData)
{
	...
	if((flags & ENTITY_BROADCAST_OTHER_CLIENT_FLAGS) > 0)
	{
		...
		for(; witer != witnesses_.end(); ++witer)
		{
			...
			// Router 模式下客户端不再与 cellapp 直连(pChannel 恒为 NULL)，
			// 此时仍可按 client Actor 直投，不能跳过。
...
void Entity::onGetWitness(bool fromBase)
{
	...
	if(fromBase)
	{
		...
		// Router 模式：该 bundle 是 legacy client 消息，走 client Actor 直投；
		// 失败(未启用 Router)则回退原有直连 sendCall。
		if(!Cellapp::getSingleton().sendBundleToClientActor(id(), pSendBundle))
...
void Entity::onTeleportRefEntityCall(EntityCall* nearbyMBRef, Position3D& pos, Direction3D& dir)
{
	...
	// 该 bundle 是"组件级裸消息"(CellappInterface::reqTeleportToCellApp)，非 newCall_ 格式：
	// Router 模式下必须作为 BODY_COMPONENT_MESSAGE 投递给目标 cellapp 组件 Actor，
	// 直接 sendCall 会被当成 BodyTag 解析而丢失。
	if(RouterMail::isEnabled())
	{
		Cellapp::getSingleton().sendMailToComponent(nearbyMBRef->componentID(),
			CELLAPP_TYPE, pBundle);
	}
	else
	{
...
void Entity::teleportLocal(PyObject_ptr nearbyMBRef, Position3D& pos, Direction3D& dir)
{
	...
	for (; witer != witnesses_.end(); ++witer)
	{
		...
		// Router 模式下客户端不再与 cellapp 直连(pChannel 恒为 NULL)，此时仍可按 client Actor 直投
		if (pChannel == NULL && !RouterMail::isEnabled())
			continue;
...
void Entity::changeToGhost(COMPONENT_ID realCell, KBEngine::MemoryStream& s)
{
	...
	// Router 模式：本进程不再是该实体的"真实体宿主"，后续投往它的 Mail 由 router 缓冲
	// (SUSPEND)。目标 cellapp 建好实体后注册同一地址，router 会自动 RESUME 并按到达顺序
	// 冲刷缓冲——这正是取代旧 forward_messagebuffer_(ghost 转发缓冲) 的机制。
	// 若迁移失败，changeToReal() 会重新注册(resume)并冲刷。
	if(RouterMail::isEnabled())
		Cellapp::getSingleton().routerSuspendActor(
			RouterMail::entityMailbox(id(), (uint32)CELLAPP_TYPE));
...
void Entity::changeToReal(COMPONENT_ID ghostCell, KBEngine::MemoryStream& s)
{
	...
	// Router 模式：本进程重新成为该实体的"真实体宿主"(通常是迁移失败回滚)。
	// 该地址此前被 changeToGhost() 置为 SUSPEND，这里注册等价于 RESUME：
	// router 把路由改回本连接，并冲刷迁移期间缓冲的 Mail，保证一条都不丢、顺序不变。
	if(RouterMail::isEnabled())
		Cellapp::getSingleton().routerRegisterActor(
			RouterMail::entityMailbox(id(), (uint32)CELLAPP_TYPE));
