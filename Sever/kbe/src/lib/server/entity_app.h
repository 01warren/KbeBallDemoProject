...
public:
	...
	*/
	...
	/**
		Router 接入钩子：entity 已加入 pEntities_、可被寻址之后回调。
		派生类可在此把该 entity 注册为 Router Actor(registerActor)。
		默认空实现，因此未接入 Router 的组件行为不变。
	*/
	virtual void onEntityCreated(E* entity) { (void)entity; }

	/**
		Router 接入钩子：entity 已从 pEntities_ 移除、即将 destroy 之前回调。
		派生类应在此注销该 entity 的 Router Actor(unregisterActor)。
	*/
	virtual void onEntityDestroyed(E* entity) { (void)entity; }
										 ...
										 bool isInitializeScript, ENTITY_ID eid, bool initProperty)
{
	...
	// Router: entity 已可被寻址，派生类在此注册 Actor
...
bool EntityApp<E>::destroyEntity(ENTITY_ID entityID, bool callScript)
{
	...
	if(entity != NULL)
	{
		// Router: entity 已从容器移除，派生类在此注销 Actor
		onEntityDestroyed(static_cast<E*>(entity.get()));
