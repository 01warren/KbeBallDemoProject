...
typedef struct EngineComponentInfo
{
	EngineComponentInfo()
	{
		...
		clusterServicePort = 20093;
		clusterPeerPort = 20094;
		clusterElectionTimeoutMS = 3000;
	...
	// ---- cluster 集群组件专属配置 ----
	std::vector< std::string > cluster_addresses;			// 集群成员(副本)地址列表(ip，本机可包含)
	uint16 clusterServicePort;								// 对组件/工具提供的TCP服务端口
	uint16 clusterPeerPort;									// cluster 副本间通信端口
	uint16 clusterElectionTimeoutMS;						// 副本选举超时(毫秒)
